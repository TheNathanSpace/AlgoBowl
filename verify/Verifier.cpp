...
//            cout << startNode << " " << weightGiven << " " << totalCost << "\n";
    ...
    if (totalCost != expectedCost) {
        std::string out =
                "total cost incorrect. expected: " + to_string(expectedCost) + ", yours: " + to_string(totalCost) +
                "\n";
        printf(out.c_str());
        return (-1);
    }
    ...
    for (int i = 0; i < numReqVertices; i++) {
        if (!visited[requiredVertices[i]]) {
            std::string out = "Required Vertex is not connected. missing: " + to_string(requiredVertices[i] + 1) + "\n";
            printf(out.c_str());
            return (-1);
        }
    }
